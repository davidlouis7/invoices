...
<body>
    ...
    @foreach ($quote->quoteItems as $i => $quoteItem)
        ...
        <div class="total" style="top: {{ strval(124.5 + (($i+1) * 7.5)) }}mm;">{{isset($quoteItem->total) ? $quoteItem->total : 'N/A'}}</div>
        @if (($i+1) == count($quote->quoteItems))
            ...
            <div class="total-num-written" style="top: {{ strval(140 + (($i+1) * 7.5)) }}mm;">إجمالي ({{\Alkoumi\LaravelArabicNumbers\Numbers::TafqeetNumber($quote->final_amount)}}) ريال سعودي</div>
